...
async function userFor(token,env){
  ...
  const u=await env.DB.prepare('SELECT id,name,role FROM users WHERE id=? AND active=1').bind(p.id).first();
  return u&&['admin','auditor','auditee'].includes(u.role)?{...u,code:u.id,admin:u.role==='admin',exp:p.exp}:null;
}
  ...
  async fetch(request,env,ctx){
    ...
    try{
      if(url.pathname==='/api/login'&&request.method==='POST'){
        ...
        const rateKey=request.headers.get('CF-Connecting-IP')||'local'; const now=Date.now();
        const rate=await env.DB.prepare('INSERT INTO login_attempts(key,count,until_at) VALUES(?,1,?) ON CONFLICT(key) DO UPDATE SET count=CASE WHEN until_at<? THEN 1 ELSE count+1 END,until_at=CASE WHEN until_at<? THEN excluded.until_at ELSE until_at END RETURNING count').bind(rateKey,now+60000,now,now).first();
        if(rate.count>10)return json({error:'Thử lại sau một phút'},429);
        const row=await env.DB.prepare('SELECT * FROM users WHERE id=? AND active=1').bind(id).first();
        if(!row||!['admin','auditor','auditee'].includes(row.role)||!await verifyPassword(password,row.password_hash))return json({error:'Sai tài khoản hoặc mật khẩu'},401);
        const token=await signToken({id:row.id,exp:now+8*3600000},env.AUTH_SECRET);
        return json({token,user:{id:row.id,code:row.id,name:row.name,role:row.role,admin:row.role==='admin'}});
      }
